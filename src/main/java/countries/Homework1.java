...
public class Homework1 {
    ...
    public boolean streamPipeline1() {
        return countries.stream().anyMatch(country -> country.getName().toUpperCase().contains("ISLAND"));
    }
    ...
    public Optional<String> streamPipeline2() {
        return countries.stream().filter(country -> country.getName().toUpperCase().contains("ISLAND")).findFirst().map(Country::getName);
    ...
    public void streamPipeline3() {
        countries.stream().filter(country -> country.getName().toUpperCase().charAt(0)==country.getName().toUpperCase().charAt(country.getName().length()-1)).map(Country::getName).forEach(System.out::println);
    }
    ...
    /**
     *  A magyar nyelvű feladat leírásban nem a 10 legkisebb lélekszámú ország népességét kell visszaadni, hanem a 10 legkisebb lélekszámú országot. Ennek megoldása:
...
ed by the language code "es").
    ...
    public void streamPipeline11() {
        countries.stream().map(Country::getTranslations).flatMap(translations -> translations.keySet().stream()).distinct().sorted().forEach(System.out::println);
    }
    ...
    public void streamPipeline15() {
        countries.stream().filter(country -> country.getArea() != null && country.getArea().floatValue() < 1).map(Country::getName).forEach(System.out::println);
    }
